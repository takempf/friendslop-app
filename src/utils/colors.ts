...
function oklchToHex(l: number, c: number, hDeg: number): string {
  ...
  const r = gamma(
    clamp(+4.0767416621 * rl - 3.3077115913 * rm + 0.2309699292 * rs),
  );
  const g = gamma(
    clamp(-1.2684380046 * rl + 2.6097574011 * rm - 0.3413193965 * rs),
  );
  const b = gamma(
    clamp(-0.0041960863 * rl - 0.7034186147 * rm + 1.707614701 * rs),
  );
...
export const COLOR_POOL = OKLCH_HUES.map((h) => oklchToHex(0.72, 0.2, h));
