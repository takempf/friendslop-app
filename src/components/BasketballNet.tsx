...
export function BasketballNet({ position: [posX, posY, posZ] }: NetProps) {
  ...
  if (ringsRef.current === null) {
    ...
    for (let i = 0; i < NUM_RINGS; i++) {
      const naturalRadius = THREE.MathUtils.lerp(TOP_RADIUS, BOTTOM_RADIUS, i / TUBE_SEGMENTS)
      state.push({
  ...
  useFrame((_, delta) => {
    ...
    for (let i = 1; i < NUM_RINGS; i++) {
      ...
      const restPos = above.center.clone().add(new THREE.Vector3(0, -RING_SPACING, 0))
      ...
      force.add(naturalPos.clone().sub(ring.center).multiplyScalar(springK * 0.2))
      ...
      // Radius spring
      const rForce = (ring.naturalRadius - ring.currentRadius) * springK * 0.5 - ring.radiusVelocity * damping
      ...
      if (localBallY > -NET_HEIGHT - BALL_RADIUS && localBallY < BALL_RADIUS) {
        ...
        for (let i = 1; i < NUM_RINGS; i++) {
          ...
          if (distY < BALL_RADIUS) {
            ...
            const halfChord = Math.sqrt(Math.max(0, BALL_RADIUS * BALL_RADIUS - distY * distY))
            
            const isInside = distXZ < ring.naturalRadius

            if (isInside) {
              // If ball pushes past current radius
              const targetRadius = Math.max(ring.naturalRadius, distXZ + halfChord)
              ...
              ring.velocity.add(new THREE.Vector3(vel.x, vel.y, vel.z).multiplyScalar(8 * dt))
            } else {
              ...
              if (ballEdgesIntoNet) {
                ...
                ring.velocity.add(new THREE.Vector3(vel.x, vel.y, vel.z).multiplyScalar(3 * dt))
              } else {
  ...
  return (
    <mesh position={[posX, posY, posZ]}>
      <cylinderGeometry args={[TOP_RADIUS, BOTTOM_RADIUS, NET_HEIGHT, RADIAL_SEGMENTS, TUBE_SEGMENTS, true]} ref={geometryRef} />
