           ...
           <div className="flex flex-col gap-1">
             ...
             <select 
               ...
               onChange={(e) => {
                 ...
                 audioManager.setInputDevice(e.target.value).catch(console.error);
               }}
             >
               {devices.filter(d => d.kind === 'audioinput').map(d => (
                 <option key={d.deviceId} value={d.deviceId}>{d.label || `Mic ${d.deviceId.slice(0, 5)}...`}</option>
               ))}
               {devices.filter(d => d.kind === 'audioinput').length === 0 && <option value="default">Default Mic</option>}
             </select>
           </div>
           ...
           <div className="flex flex-col gap-1">
             ...
             <select 
               ...
               onChange={(e) => {
                 ...
                 audioManager.setOutputDevice(e.target.value).catch(console.error);
               }}
             >
               {devices.filter(d => d.kind === 'audiooutput').map(d => (
                 <option key={d.deviceId} value={d.deviceId}>{d.label || `Speaker ${d.deviceId.slice(0, 5)}...`}</option>
               ))}
               {devices.filter(d => d.kind === 'audiooutput').length === 0 && <option value="default">Default Speaker</option>}
             </select>
           </div>
      ...
      <div className="flex-1 min-h-[200px] bg-black/40 rounded-md flex flex-col border border-zinc-800">
        <div className="flex-1 p-3 overflow-y-auto flex flex-col gap-1 text-sm">
          {chatMessages.length === 0 && (
             <div className="text-gray-500 italic text-center mt-4">No messages yet...</div>
          )}
        ...
        <form onSubmit={handleSend} className="border-t border-gray-600 p-2 flex shrink-0">
          <input
            type="text"
